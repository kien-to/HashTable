...
public class ArrayHashTable<K, V> implements HashTable<K, V> {
  ...
  protected boolean[] emptyAfterRemoval;
  ...
  public ArrayHashTable(CollisionHandler <K> collisionHdler) {
      ...
      this.emptyAfterRemoval = new boolean[capacity];
  }
  ...
  private void resizeArray() {
   //TODO: Implement this method
    ...
    // boolean[] newEmptyArray = new boolean[capacity];
    int bucket = 0;
    int i = 0;
    while (i < capacity){
    ...
    while (bucket < currentSize) {
      if (isActiveArray[bucket]) {
        int index = getHash(keyArray[bucket]);
        int newIndex = collisionHandler.doProbe(index, newActiveArray, capacity);
        newKeyArray[newIndex] = keyArray[bucket]; 
        newValueArray[newIndex] = valueArray[bucket];
        newActiveArray[newIndex] = true;
      }
      bucket++; 
    }
  ...
  */
  ...
  public void put(K key, V value) {
      ...
      int index = searchIndex(0, key);
      if (index != -1){
        valueArray[index] = value;
      } 
      else {
        int actualIndex = getHash(key);
        int handleCollision = resolveCollision(actualIndex);
        keyArray[handleCollision] = key;
        valueArray[handleCollision] = value;
        isActiveArray[handleCollision] = true;
        // emptyAfterRemoval[actualIndex] = false;
      }
      count++;
  }
  ...
  public V get(K target) {
   //TODO: Implement this method
      ...
      int actualIndex = searchIndex(index, target);
      if (actualIndex != -1)
        return valueArray[actualIndex];
      return null;
  }
  ...
  public V remove(K targetKey)throws ElementNotFoundException {
   //TODO: Implement this method
    ...
    int actualIndex = searchIndex(index, targetKey);
    if (actualIndex == -1)
      throw new ElementNotFoundException("");
    V value = get(targetKey);
    isActiveArray[actualIndex] = false;
    // emptyAfterRemoval[actualIndex] = true;
    count--;
    return value;
  }
